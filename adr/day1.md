...
### Запуск и исследование прототипа микросервисной архитектуры

Данное руководство описывает процесс запуска и исследования прототипа микросервисной архитектуры на базе Python с использованием gRPC и Docker. Прототип включает в себя три микросервиса (заказы, резервирование и лояльность) и API-шлюз (оркестратор) для координации запросов.
...
Для обеспечения целостности данных и соблюдения бизнес-логики рекомендуется использовать запросы через API-шлюз (HTTP порт 8080), который обеспечивает корректную последовательность вызовов и применение всех необходимых проверок.
